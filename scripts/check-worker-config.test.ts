...
describe("worker config preflight", () => {
  ...
  it("reports the exact unresolved production placeholders", () => {
    ...
    // The template's own placeholders are hyphenated so wrangler's dry-run
    // accepts their shape; they must still register as unresolved.
    config.env.production.d1_databases[0]!.database_id = "your-d1-database-id";
    config.env.production.r2_buckets[0]!.bucket_name = "your-r2-bucket-name";

    expect(validateWorkerConfig(config, "production")).toEqual(expect.arrayContaining([
      expect.stringContaining("your-d1-database-id"),
      expect.stringContaining("your-r2-bucket-name"),
    ]));
  });
