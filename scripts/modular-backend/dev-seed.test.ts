...
describe("shared local development seed", () => {
  it("populates a fresh VPS database and remains idempotent", async () => {
    ...
    try {
      expect(count(database, "users")).toBe(10);
      ...
      expect(count(database, "events")).toBe(10);
      ...
      expect(count(database, "guild_wars")).toBe(4);
    ...
    try {
      database.exec(`
        INSERT INTO users (
          ...
          'existing-owner', 'existing_owner', 'site_owner', 1, NULL,
