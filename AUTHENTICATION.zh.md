...
> 状态：Phase 0–3 已在当前工作区实现，但尚未提交、部署或发布。[README.zh.md](./README.zh.md)、[SETUP.zh.md](./SETUP.zh.md) 与 [SECURITY.md](./SECURITY.md) 仍是运行参考。
>
...
| --- | --- |
| 成员账号创建 | 继续仅限邀请。 |
| 注册 | 邀请码、私密登录名、公开显示名、密码、确认密码；不出现 OAuth、邮箱或手机步骤。 |
...
把公开显示名与私密登录名分开是有价值的，但登录名不是秘密，也不是第二因素。只有成员选择了不再公开显示的登录值后，它才能降低针对成员名册进行的机会型密码猜测和撞库；迁移账号最初仍保留已经公开过的旧值。真正的安全控制仍然是强密码、限流、锁定处理、会话安全和可靠恢复。邀请注册限制的是未经授权的新账号创建，并不能阻止对已有账号的攻击。
...
1. 公会管理员按现有邀请策略创建邀请。
2. 注册页要求输入邀请码、登录名、显示名、密码与确认密码。
3. 服务端先校验邀请，再执行名称冲突检查或密码哈希。
4. 一个事务同时消费邀请、创建内部用户、创建本地凭据、创建个人资料并写入注册审计。名称冲突或邀请失效时不消费任何内容。
...
| `login_failures` | 按标准化私密登录名保存持久渐进锁定状态，并在查询账号和执行密码派生之前检查。 |
...
当前代码使用自描述 PBKDF2-HMAC-SHA256 哈希，并以 10,000 次作为兼容下限，因为 Cloudflare Workers Free 的 CPU 预算受限。文档应把这个数字表述为部署约束，不能称为现代最佳强度。
...
- 永远不降低已保存哈希的成本；
- 新密码为 8 至 128 个字符，允许空格和 Unicode，不使用字符种类组合规则或弱密码匹配；
- 现有哈希保留到成员下次修改/重置密码，升级时不批量锁定成员。

如果站主启用面向任意收件人的 Cloudflare Email Sending，则该 Cloudflare 账号已经使用 Workers Paid，应基准测试更高的 PBKDF2 配置。VPS 站主也应如此。未来更换密码哈希算法必须保持运行时中立，并沿用自描述迁移路径；Cloudflare 和 VPS 不得静默使用不同的认证强度。

### 暴力尝试与枚举防护

- 在昂贵工作之前同时应用 IP 总限流，以及客户端/IP 加登录名组合限流。
- 在账号查询和 PBKDF2 之前检查活跃的持久登录锁。
- 请求未锁定时，对不存在或不可用账号继续执行等形态的 dummy 密码派生。
- 保留持久渐进退避、统一错误、有界保留期、权限控制的锁定重置与审计。
...
保留 HTTP-only、same-site Cookie、滚动和绝对过期、会话数量上限、CSRF/Origin 防护，并用认证版本在凭据或账号状态改变后使旧会话失效。HTTPS 固定使用 `__Host-ig_session` 与 `__Host-ig_session_oauth_transaction`，两者均带 `Secure`、`Path=/` 且无 `Domain`；纯 HTTP 本地开发使用无前缀 Cookie。OAuth 回调仍验证 state 与浏览器 Cookie。会话、邀请、OAuth、验证和临时凭据 token 都不得写入日志。
...
2. 把每条当前 `users.username` 原样复制到这两个字段。现有密码哈希不变，当前登录失败状态继续使用相同键值。
...
7. 把持久登录锁的归属改为 `login_name`；登录名改变时原子清理/迁移锁定状态。
...
- 把活跃锁定拒绝移到账号查询和 PBKDF2 之前，与仓库安全契约一致。
- 增加 IP 总限流和 IP/登录名组合限流，同时保留持久渐进锁定和 dummy 派生。
- 为注册/修改/重置增加 8 至 128 个字符的密码政策、统一失败响应与测试。
- 管理员重置时撤销会话与 OAuth 连接；要求管理员当前密码、临时到期、单次/强制修改状态、认证版本旋转和原子审计。

**退出条件：** 聚焦的 service/store/route 测试通过；锁定请求既不查账号也不运行 PBKDF2；两个运行时执行完全相同行为。
...
- 事务、唯一约束、锁定、重放、会话撤销和审计原子性的 service 测试；
