...
vi.mock("react-i18next", () => ({
  useTranslation: () => ({
    t: (key: string, options?: { count?: number; defaultValue?: string }) => {
      ...
      if (key === "audit.entityType.site_config") return "Site Config";
...
function makeAuditRow(id: string, detail: JsonObject | null): AuditLogEntry {
  return {
    id,
...
describe("AuditLogViewer", () => {
  ...
  it("renders actor usernames from audit rows instead of raw actor ids", () => {
    ...
    expect(screen.getByText(/GuildAdmin Uploaded Site Config/)).toBeInTheDocument();
    expect(screen.queryByText(/admin-user-id Uploaded Site Config/)).not.toBeInTheDocument();
  });
  ...
  it("mounts a 20KB info value only after the user expands it", async () => {
    ...
    renderAuditLogViewer([makeAuditRow("audit-info", { notes: longValue })]);

    await user.click(screen.getByRole("button", { name: /GuildAdmin Uploaded Site Config/ }));
  ...
  it("limits a 20KB diff value before explicit expansion", async () => {
    ...
    renderAuditLogViewer([
      makeAuditRow("audit-diff", { description: { from: "short", to: longValue } }),
    ]);
  ...
  it("renders short values unchanged without an expansion control", async () => {
    const user = userEvent.setup();
    renderAuditLogViewer([makeAuditRow("audit-short", { notes: "short value" })]);
