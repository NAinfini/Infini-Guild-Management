...
import { resolveClassCatalogItem } from "@portal/stores/class-catalog";
 ...
 * 全名挂在 title 上——鼠标停一下就是完整一份，一个都没丢。
 */
const MAX_VISIBLE = 6;
...
export function ClassIconStrip({ classIds, catalog, emptyLabel }: ClassIconStripProps) {
  ...
  return (
    <span className="quota-editor__strip" title={items.map((item) => item.label).join(", ")}>
