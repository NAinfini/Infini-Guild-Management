 ...
 * 本地时间和落库的 *_utc 一一对应，断言才写得成固定值。共享契约允许 24:00 作为
 * 当天末端，服务端会原样保存；一周没有任何时段时则规范化为 availability: null。
 *
 * 每条用例先保存 admin 当前的 availability，再通过真实资料接口清空，确保编辑器
 * 总是从空的一周开始；afterEach 用一次 PATCH 把原值写回。
 */
...
test("预设：叠加而不是覆盖，清空一键归零，两次都要保存才落库", async ({ page, flow, api }) => {
  for (const day of ["Mon", "Sat"]) {
    await expect(blocksOf(page, day), "用例开始前已经显式清空时段").toHaveCount(0);
  }
  ...
  expect(saved.monday, "当天末端按共享契约保留为 24:00").toEqual([{ start_utc: "20:00", end_utc: "24:00" }]);
  expect(saved.saturday).toEqual([{ start_utc: "10:00", end_utc: "24:00" }]);
  ...
  await save(page, flow);
  expect((await readProfile(api)).availability, "清空之后服务端用 null 表示整周没有时段").toBeNull();
});
...
test("单日时段：结束早于开始就不让加，加完能删，两步都要保存才落库", async ({ page, flow, api }) => {
  ...
  await save(page, flow);
  expect((await readProfile(api)).availability, "删掉最后一段后服务端应回到空值").toBeNull();
});
...
test("复制到某天：空的一天不能复制，复制过去的时段能落库", async ({ page, flow, api }) => {
  ...
  await save(page, flow);
  const saved = await readAvailability(api);
  expect(saved.tuesday).toEqual([{ start_utc: "20:00", end_utc: "24:00" }]);
