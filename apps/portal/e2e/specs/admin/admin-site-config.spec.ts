 ...
 *
 * 唯独站点标志的上传没有测：
 * 上传成功时服务端会顺手把旧标志的 R2 对象删掉（SiteConfigService.ts:258），
 * 而 updateSiteConfigSchema 里根本没有 site_logo_url 这一项（schemas/site-config.ts:100），
 * 也就是说这个动作既删得掉旧图、又还原不回旧地址——是一扇单向门。
 * 在共享的本地库上跑一次就把种子标志弄丢了，所以这里不碰它；
 * 真要覆盖，得先给它补一个「恢复默认标志」的接口。
 */
