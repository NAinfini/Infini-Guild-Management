 ...
 * 所以断言必须落到服务端存下来的 icon_type / icon_media_id，并读取 full/view 两个变体。
 */
...
test("新建职业：没名字存不了；填完之后颜色、图标原样落库，新的排在末尾，清单和计数一起更新", async ({ page, api, flow }) => {
  ...
  expect(saved.icon_media_id, "矢量图标不该关联图片媒体").toBeNull();
...
test("自定义图标：不选图存不了；传完落到 R2 且取得回来；再切回矢量会把图删干净", async ({ page, api, flow }) => {
  ...
  expect(withIcon.icon_media_id, "服务端必须关联统一媒体记录，否则清单只能显示裂图").toBeTruthy();
  await expect(classItem(page, item.label)).toContainText("Custom image");

  /* 只断言库里存了个 id 不够；快速显示和显式原图都必须取得回来。 */
  const mediaId = withIcon.icon_media_id as string;
  for (const variant of ["view", "full"] as const) {
    const fetched = await api.get(`/api/media/${mediaId}/${variant}`);
    expect(fetched.status(), `${variant} 图标应当能取到`).toBe(200);
