 ...
 * 上传尤其不能只看界面：库里多一行不等于 R2 里多一个对象，所以上传用例会拿返回的
 * 对象键回头去 /api/gallery/image 把字节取回来——取不回来就是半成功，
 * 这种状态在界面上和成功一模一样。
 *
...
async function uploadImage(api: APIRequestContext, name: string): Promise<Fixture> {
  ...
  const uploaded = await readJson(
    await api.post("/api/gallery/images", {
      multipart: {
        files: webpUpload(`gallery-${name.toLowerCase()}-${stamp}.webp`),
        captions: caption,
      },
    }),
    `上传图片 ${caption}`,
...
test("图片上传：说明随图提交，库里有行、R2 里也要有对象", async ({ page, flow, api }) => {
  ...
  const object = await api.get(`/api/gallery/image?key=${encodeURIComponent(item.url)}`);
  expect(object.status(), "库里有行不等于 R2 里有对象，字节必须真的取得回来").toBe(200);
