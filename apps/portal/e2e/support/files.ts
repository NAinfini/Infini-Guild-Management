...
/** Direct API equivalent of the browser's canonical full/view image upload. */
export function imageVariantsUpload(name: string) {
  return {
    full: webpUpload(`${name}-full.webp`),
    view: webpUpload(`${name}-view.webp`),
  };
}
