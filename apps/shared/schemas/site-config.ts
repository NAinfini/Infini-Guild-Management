...
export const siteMediaPolicySchema = z.object({
  max_file_size_bytes: z.object({
    profile_image: z.number().int().positive(),
    profile_audio: z.number().int().positive(),
    announcement_image: z.number().int().positive(),
    wiki_image: z.number().int().positive(),
    event_image: z.number().int().positive(),
    gallery_image: z.number().int().positive(),
  }),
  quotas: z.object({
    profile: z.number().int().positive(),
    announcement: z.number().int().positive(),
    gallery: z.number().int().positive(),
    wiki: z.number().int().positive(),
  }),
});

export const sitePaginationPolicySchema = z.object({
  admin: z.number().int().positive(),
  announcements: z.number().int().positive(),
  events: z.number().int().positive(),
  gallery: z.number().int().positive(),
  guild_war: z.number().int().positive(),
  users: z.number().int().positive(),
  wiki: z.number().int().positive(),
});

export const siteStoragePolicySchema = z.object({
  images_per_item: z.number().int().positive(),
});
...
export const DEFAULT_SITE_PAGINATION_POLICY = sitePaginationPolicySchema.parse({
  admin: LIMITS.pagination.admin,
  announcements: LIMITS.pagination.announcements,
  events: LIMITS.pagination.events,
  gallery: LIMITS.pagination.gallery,
  guild_war: LIMITS.pagination.guildWar,
  users: LIMITS.pagination.users,
  wiki: LIMITS.pagination.wiki,
});
...
export const siteConfigSchema = z.object({
  ...
  pagination_policy: sitePaginationPolicySchema,
...
export const updateSiteConfigSchema = z.object({
  ...
  analytics_settings: siteAnalyticsSettingsSchema.partial().optional(),
...
export type SitePaginationPolicy = z.infer<typeof sitePaginationPolicySchema>;
