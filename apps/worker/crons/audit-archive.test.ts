...
describe("audit archive manifest commit protocol", () => {
  ...
  it("writes and verifies every data part, commits the manifest last, then deletes D1 rows", async () => {
    const rows = Array.from({ length: 1001 }, (_, index) => row(String(index).padStart(4, "0")));
    rows[0] = { ...rows[0]!, detail_text: '{"changed":true}' };
    const fixture = envWith(rows);
    ...
    const firstDataKey = fixture.bucket.putOrder.find((key) => key.endsWith(".ndjson.gz"))!;
    const archivedText = await new Response(
      new Blob([fixture.bucket.objects.get(firstDataKey)!.bytes])
        .stream()
        .pipeThrough(new DecompressionStream("gzip")),
    ).text();
    const archivedRow = JSON.parse(archivedText.split("\n")[0]!) as Record<string, unknown>;
    expect(archivedRow.detail).toEqual({ changed: true });
    expect(archivedRow).not.toHaveProperty("detail_text");
