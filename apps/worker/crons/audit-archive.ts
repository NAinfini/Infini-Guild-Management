import { auditLogSchema, type AuditLogEntry } from "@guild/shared";
import type { Bindings } from "../index";
import { parseJsonObject } from "../services/json-object-codec";
...
function toNdjson(rows: AuditArchiveDbRow[]): string {
  return rows.map((row) => {
    const { detail_text: detailText, ...fields } = row;
    return JSON.stringify(auditLogSchema.parse({
      ...fields,
      detail: parseJsonObject(detailText),
    }));
  }).join("\n");
}
...
async function readAndVerifyDataFile(
  ...
  return lines.map((line) => auditLogSchema.parse(JSON.parse(line) as unknown));
}

async function readAndVerifyManifest(bucket: R2Bucket, month: string): Promise<{ manifest: AuditArchiveManifest; rows: AuditLogEntry[] } | null> {
