...
-- that already recorded those migrations are unaffected: their names stay in
-- d1_migrations and this baseline is recorded as applied, so nothing re-runs.
-- A fresh database reaches the identical schema in one step.
--
...
INSERT OR IGNORE INTO "class_catalog" ("id", "label", "color", "icon_type", "vector_icon", "icon_key", "sort_order") VALUES
...
INSERT OR IGNORE INTO "onboarding_config" ("id", "title", "body_json", "checklist_json", "require_ack", "published_at", "updated_by") VALUES
  ('default', 'Member onboarding', '{"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"Welcome to the guild. Review the rules before joining events, claiming storage, or participating in guild war planning."}]},{"type":"paragraph","content":[{"type":"text","text":"Keep your profile current and contact leadership when your availability, name, role, or class changes."}]}]}', '[{"id":"read-rules","label":"Read guild rules","description":"Understand expectations for events, storage, guild war, and member communication.","required":true},{"id":"complete-profile","label":"Complete your profile","description":"Add your power, class, availability, and contact details.","required":true},{"id":"ask-questions","label":"Ask questions early","description":"Contact leadership if any rule or workflow is unclear.","required":false}]', 1, NULL, NULL);
