-- Immutable production baseline captured from the schema-only D1 export.
-- Production-only tables remain part of this historical shape and are preserved by later migrations.
...
CREATE TABLE error_log (id TEXT PRIMARY KEY NOT NULL, source TEXT NOT NULL, level TEXT NOT NULL DEFAULT 'error', message TEXT NOT NULL, request_path TEXT, request_method TEXT, request_id TEXT, stack TEXT, context TEXT, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')));
CREATE TABLE member_badges (id TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, label_html TEXT NOT NULL, color TEXT NOT NULL DEFAULT '#3b82f6', description TEXT, sort_order INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')), updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')));
CREATE TABLE users (
  id TEXT PRIMARY KEY NOT NULL,
...
CREATE TABLE event_polls (event_id TEXT PRIMARY KEY NOT NULL REFERENCES events(id) ON DELETE CASCADE, results_visibility TEXT NOT NULL DEFAULT 'after_vote' CHECK (results_visibility IN ('always', 'after_vote', 'after_close')), show_voter_names INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')), updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')));
CREATE TABLE event_poll_options (id TEXT PRIMARY KEY NOT NULL, event_id TEXT NOT NULL REFERENCES events(id) ON DELETE CASCADE, label TEXT NOT NULL, sort_order INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')));
CREATE TABLE game_data (id INTEGER PRIMARY KEY AUTOINCREMENT, data TEXT NOT NULL, version TEXT NOT NULL, uploaded_by TEXT NOT NULL REFERENCES users(id), created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')));
...
CREATE TABLE member_onboarding_state (
  user_id TEXT PRIMARY KEY NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_wiki_categories_parent_sort
...
CREATE INDEX idx_announcements_status_pinned_created
  ON announcements(status, pinned, created_at, id);
CREATE INDEX idx_announcements_schedule
  ON announcements(status, publish_at);
CREATE INDEX idx_announcements_expiry
  ON announcements(status, expires_at);
CREATE INDEX idx_audit_log_created_at
  ON audit_log(created_at);
CREATE INDEX idx_audit_log_entity_actor_created
...
CREATE UNIQUE INDEX ux_events_series_instance
...
CREATE INDEX idx_war_history_event_id
  ON war_history(event_id);
CREATE INDEX idx_war_history_event_created
  ON war_history(event_id, created_at, id);
CREATE INDEX idx_war_history_created
  ON war_history(created_at, id);
CREATE INDEX idx_war_teams_history_sort
  ON war_teams(war_history_id, sort_order, id);
CREATE INDEX idx_war_teams_event_sort
  ON war_teams(event_id, sort_order, id);
CREATE INDEX idx_wiki_articles_category_archived_sort
  ON wiki_articles(category_id, archived_at, pinned, sort_order, updated_at, id);
CREATE INDEX idx_wiki_articles_archived_updated
...
CREATE INDEX idx_game_data_created_at ON game_data(created_at);
...
CREATE INDEX idx_storage_items_storage_category ON storage_items(storage_id, category_id);
CREATE INDEX idx_storage_item_images_item ON storage_item_images(item_id);
CREATE INDEX idx_storage_transactions_item ON storage_transactions(item_id, created_at);
CREATE INDEX idx_storage_transactions_recipient ON storage_transactions(recipient_user_id, created_at) WHERE recipient_user_id IS NOT NULL;
