...
beforeEach(() => {
  ...
  mocks.media.getMediaPolicy.mockReset().mockResolvedValue({
    max_file_size_bytes: { class_icon: 5_000_000 },
  });
});
...
describe("class catalog routes", () => {
  ...
  it("returns 400 for an explicit icon validation result", async () => {
    ...
    form.set("full", new File(["webp"], "full.webp", { type: "image/webp" }));
    form.set("view", new File(["webp"], "view.webp", { type: "image/webp" }));
  ...
  it("lets storage failures reach the global 500 path", async () => {
    ...
    const form = new FormData();
    form.set("full", new File(["RIFF____WEBP"], "full.webp", { type: "image/webp" }));
    form.set("view", new File(["RIFF____WEBP"], "view.webp", { type: "image/webp" }));
