...
export const SYSTEM_TEST_ARTIFACT_TYPES = [
  ...
  "storage", "storage_category", "storage_item", "storage_batch", "audit_log", "error_log", "media_asset",
...
export class SystemTestService {
  ...
  async cleanupExactArtifacts(artifacts: readonly SystemTestArtifact[]): Promise<void> {
    ...
    const mediaAssetIds = byType.get("media_asset") ?? [];
    ...
    await deleteById("storage_batches", "storage_batch");
    ...
    await new MediaService(this.env.DB, this.env.MEDIA).deleteAssets(mediaAssetIds);
  }
  ...
  async finalizeRun(
    runId: string,
    actorId: string,
    summary?: { diffTitle: string; detail: JsonObject },
  ): Promise<void> {
    ...
    const results = await this.env.DB.batch([
      ...
      ).bind(auditId, summary.diffTitle, serializeJsonObject(summary.detail), runId, actorId),
      this.env.DB.prepare(
        `DELETE FROM system_test_runs WHERE ${completedRunPredicate}`,
      ).bind(runId, actorId),
    ]);
  ...
  private async deleteRegisteredArtifacts(runId: string): Promise<void> {
    ...
    const mediaAssetIds = keys("media_asset");
    ...
    await deleteById("storage_batches", keys("storage_batch"));
