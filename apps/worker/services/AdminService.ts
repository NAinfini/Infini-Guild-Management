...
function parsePermissionRecord(
  permissionRows: Array<{ permission: string }>,
...
async function replaceRolePermissions(
  ...
  for (const perm of PERMISSIONS) {
    if (permissionRecord[perm]) {
      stmts.push(rawDb.prepare("INSERT INTO role_permissions (role_id, permission) VALUES (?1, ?2)").bind(roleId, perm));
    }
  }
...
export class AdminService {
  ...
  async listInviteLinks(
    ...
    const rows = await this.deps.db
      ...
      .from(inviteLinks)
      .innerJoin(roles, eq(inviteLinks.roleId, roles.id))
  ...
  async createInviteLink(actorId: string, roleId: string, maxUses: number, expiresAt: string | null): Promise<ServiceResult<unknown>> {
    const assignment = await this.assertRoleAssignable(actorId, roleId);
    if (!assignment.ok) return assignment.error;
    const inviteId = this.deps.generateId();
    const code = this.deps.generateInviteCode();
    await this.deps.db.insert(inviteLinks).values({ id: inviteId, code, createdBy: actorId, roleId, maxUses, usedCount: 0, expiresAt, revokedAt: null });
    const created = (await this.deps.db.select({ id: inviteLinks.id, code: inviteLinks.code, createdBy: inviteLinks.createdBy, roleId: inviteLinks.roleId, roleName: roles.name, roleColor: roles.color, roleLevel: roles.level, maxUses: inviteLinks.maxUses, usedCount: inviteLinks.usedCount, expiresAt: inviteLinks.expiresAt, createdAt: inviteLinks.createdAt, revokedAt: inviteLinks.revokedAt }).from(inviteLinks).innerJoin(roles, eq(inviteLinks.roleId, roles.id)).where(eq(inviteLinks.id, inviteId)).limit(1))[0];
    if (!created) return err("SERVER_ERROR", "Failed to create invite link");
    await this.deps.writeAuditLog({ entityType: "invite_link", action: "create", actorId, entityId: inviteId, diffTitle: inviteId, detail: { role_id: roleId, max_uses: maxUses, expires_at: expiresAt } });
    return ok(inviteLinkSchema.parse({ id: created.id, code: created.code, created_by: created.createdBy, role_id: created.roleId, role_name: created.roleName, role_color: created.roleColor, role_level: created.roleLevel, max_uses: created.maxUses, used_count: created.usedCount, expires_at: created.expiresAt, created_at: created.createdAt, revoked_at: created.revokedAt }));
  }
  ...
  async batchUpdateRole(actorId: string, userIds: string[], newRoleId: string): Promise<ServiceResult<{ updated: number }>> {
    ...
    const escalated = await this.findEscalatedRoleGrants(actorRole.roleId, newRoleId);
    if (escalated.length > 0) return err("FORBIDDEN", `Cannot assign a role with permissions you do not hold: ${escalated.join(", ")}`);
    ...
    await this.deps.writeAuditLogDurable({ entityType: "user", action: "batch_role_update", actorId, entityId: "batch", diffTitle: usernames.join(", "), detail: { user_ids: targetIds, usernames, new_role: newRoleId, count: existingUsers.length } });
  ...
  async batchDeactivate(actorId: string, userIds: string[]): Promise<ServiceResult<{ updated: number }>> {
    return this.executeBatchAction(actorId, userIds, {
      ...
      clearSessions: true, durable: true,
  ...
  async batchReactivate(actorId: string, userIds: string[]): Promise<ServiceResult<{ updated: number }>> {
    return this.executeBatchAction(actorId, userIds, {
      ...
      clearSessions: false, durable: false,
  ...
  private async executeBatchAction(
    ...
    const guard = await this.assertBatchActionAllowed(actorId, targetIds);
    if (!guard.ok) return guard.error;
    const existingUsers = guard.existingUsers;
    ...
    await writeLog({ entityType: "user", action: opts.action, actorId, entityId: "batch", diffTitle: usernames.join(", "), detail: { user_ids: targetIds, usernames, count: existingUsers.length } });
    return ok({ updated: existingUsers.length });
  }

  async createMember(actorId: string, username: string, roleId: string): Promise<ServiceResult<{ user_id: string; username: string; temporary_password: string }>> {
    ...
    try {
      await this.deps.rawDb.batch([
        this.deps.rawDb.prepare("INSERT INTO users (id, username, role, is_active, deleted_at, created_at, updated_at) VALUES (?1, ?2, ?3, 1, NULL, ?4, ?4)").bind(userId, username, roleId, nowIso),
        ...
        this.deps.rawDb.prepare("INSERT INTO member_profiles (user_id, power) VALUES (?1, 0)").bind(userId),
      ]);
    } catch (error) {
    ...
    await this.deps.writeAuditLog({ entityType: "user", action: "admin_create_member", actorId, entityId: userId, diffTitle: username, detail: { username, role_id: roleId } });
  ...
  async updateUserRole(actorId: string, targetUserId: string, newRoleId: string): Promise<ServiceResult<void>> {
    ...
    const escalated = await this.findEscalatedRoleGrants(actorRole.roleId, newRoleId);
    if (escalated.length > 0) return err("FORBIDDEN", `Cannot assign a role with permissions you do not hold: ${escalated.join(", ")}`);
    const target = (await this.deps.db.select({ id: users.id, role: users.role, roleLevel: roles.level, deletedAt: users.deletedAt, username: users.username }).from(users).innerJoin(roles, eq(users.role, roles.id)).where(eq(users.id, targetUserId)).limit(1))[0];
    if (!target || target.deletedAt !== null) return err("NOT_FOUND", "User not found");
    if (target.roleLevel >= actorRole.level) return err("FORBIDDEN", "Cannot change the role of a user at or above your own level");
    ...
    await this.deps.writeAuditLogDurable({ entityType: "user", action: "update_role", actorId, entityId: targetUserId, diffTitle: target.username, detail: { role: { from: target.role, to: newRoleId } } });
    return ok(undefined);
  }
  ...
  async deactivateUser(actorId: string, targetUserId: string, reason?: string): Promise<ServiceResult<void>> {
    ...
    const actorRole = await this.getActorRoleLevel(actorId);
    if (!actorRole) return err("FORBIDDEN", "Could not resolve actor role");
    const target = (await this.deps.db.select({ id: users.id, isActive: users.isActive, deletedAt: users.deletedAt, username: users.username, roleLevel: roles.level }).from(users).innerJoin(roles, eq(users.role, roles.id)).where(eq(users.id, targetUserId)).limit(1))[0];
    if (!target || target.deletedAt !== null) return err("NOT_FOUND", "User not found");
    if (target.roleLevel >= actorRole.level) return err("FORBIDDEN", "Cannot deactivate a user at or above your own level");
    ...
    await this.deps.writeAuditLogDurable({ entityType: "user", action: "deactivate", actorId, entityId: targetUserId, diffTitle: target.username, detail: { reason: reason ?? null } });
    return ok(undefined);
  }

  async reactivateUser(actorId: string, targetUserId: string, reason?: string): Promise<ServiceResult<void>> {
    if (targetUserId === actorId) return err("CONFLICT", "You cannot reactivate yourself");
    const actorRole = await this.getActorRoleLevel(actorId);
    if (!actorRole) return err("FORBIDDEN", "Could not resolve actor role");
    const target = (await this.deps.db.select({ id: users.id, isActive: users.isActive, deletedAt: users.deletedAt, username: users.username, roleLevel: roles.level }).from(users).innerJoin(roles, eq(users.role, roles.id)).where(eq(users.id, targetUserId)).limit(1))[0];
    if (!target || target.deletedAt !== null) return err("NOT_FOUND", "User not found");
    if (target.roleLevel >= actorRole.level) return err("FORBIDDEN", "Cannot reactivate a user at or above your own level");
    ...
    await this.deps.writeAuditLog({ entityType: "user", action: "reactivate", actorId, entityId: targetUserId, diffTitle: target.username, detail: { reason: reason ?? null } });
    return ok(undefined);
  }
  ...
  async resetPassword(actorId: string, targetUserId: string, temporaryPasswordInput?: string): Promise<ServiceResult<{ temporary_password: string }>> {
    ...
    const actorRole = await this.getActorRoleLevel(actorId);
    if (!actorRole) return err("FORBIDDEN", "Could not resolve actor role");
    const target = (await this.deps.db.select({ id: users.id, deletedAt: users.deletedAt, username: users.username, roleLevel: roles.level }).from(users).innerJoin(roles, eq(users.role, roles.id)).where(eq(users.id, targetUserId)).limit(1))[0];
    if (!target || target.deletedAt !== null) return err("NOT_FOUND", "User not found");
    if (target.roleLevel >= actorRole.level) return err("FORBIDDEN", "Cannot reset password for a user at or above your own level");
    const passwordHash = await this.deps.createPasswordHash(temporaryPassword);
  ...
  async resetLoginLock(actorId: string, targetUserId: string): Promise<ServiceResult<{ ok: true }>> {
    ...
    const target = (await this.deps.db.select({ id: users.id, deletedAt: users.deletedAt, username: users.username, roleLevel: roles.level }).from(users).innerJoin(roles, eq(users.role, roles.id)).where(eq(users.id, targetUserId)).limit(1))[0];
    if (!target || target.deletedAt !== null) return err("NOT_FOUND", "User not found");
    if (target.roleLevel >= actorRole.level) return err("FORBIDDEN", "Cannot clear the login lock for a user at or above your own level");
  ...
  async listRoles(): Promise<ServiceResult<AdminRole[]>> {
    const roleRows = await this.deps.db.select({ id: roles.id, name: roles.name, level: roles.level, color: roles.color, createdAt: roles.createdAt, updatedAt: roles.updatedAt }).from(roles).orderBy(desc(roles.level), roles.name);
    const roleIds = roleRows.map((r) => r.id);
    const permissionRows = roleIds.length > 0 ? await this.deps.db.select({ roleId: rolePermissions.roleId, permission: rolePermissions.permission }).from(rolePermissions).where(inArray(rolePermissions.roleId, roleIds)) : [];
    ...
    return ok(roleRows.map((r) => adminRoleSchema.parse({ id: r.id, name: r.name, level: r.level, color: r.color, created_at: r.createdAt, updated_at: r.updatedAt, permissions: parsePermissionRecord(permissionRows.filter((p) => p.roleId === r.id)), assigned_user_count: assignedCountByRole.get(r.id) ?? 0 })));
  }
  ...
  async createRole(actorId: string, input: { id?: string; name: string; level: number; color?: string; permissions?: Record<string, boolean> }): Promise<ServiceResult<unknown>> {
    ...
    await this.deps.db.insert(roles).values({ id: roleId, name: input.name.trim(), level: input.level, color: input.color ?? null });
    await replaceRolePermissions(this.deps.rawDb, roleId, permissionRecord);
    await this.deps.writeAuditLogDurable({ entityType: "role", action: "create", actorId, entityId: roleId, diffTitle: input.name.trim(), detail: { name: input.name.trim(), level: input.level, color: input.color ?? null, permissions: permissionRecord } });
    const created = (await this.deps.db.select({ id: roles.id, name: roles.name, level: roles.level, color: roles.color, createdAt: roles.createdAt, updatedAt: roles.updatedAt }).from(roles).where(eq(roles.id, roleId)).limit(1))[0];
    if (!created) return err("SERVER_ERROR", "Failed to create role");
    return ok(adminRoleSchema.parse({ id: created.id, name: created.name, level: created.level, color: created.color, created_at: created.createdAt, updated_at: created.updatedAt, permissions: permissionRecord, assigned_user_count: 0 }));
  }
  ...
  async updateRole(actorId: string, roleId: string, input: { name?: string; level?: number; color?: string | null; permissions?: Record<string, boolean> }): Promise<ServiceResult<AdminRole>> {
    ...
    const existing = (await this.deps.db.select({ id: roles.id, name: roles.name, level: roles.level, color: roles.color }).from(roles).where(eq(roles.id, roleId)).limit(1))[0];
    if (!existing) return err("NOT_FOUND", "Role not found");
    const actorRole = await this.getActorRoleLevel(actorId);
    if (!actorRole) return err("FORBIDDEN", "Could not resolve actor role");
    const isOwnRole = existing.id === actorRole.roleId;
    if (!isOwnRole && existing.level >= actorRole.level) return err("FORBIDDEN", "Cannot edit a role at or above your own level");
    if (input.level !== undefined) {
      if (isOwnRole && input.level > existing.level) return err("VALIDATION_ERROR", "Cannot raise the level of your own role");
      if (!isOwnRole && input.level >= actorRole.level) return err("VALIDATION_ERROR", `Role level must be below your own (${actorRole.level})`);
    }
    ...
    if (input.permissions) {
      const currentPermissionRows = await this.deps.db.select({ permission: rolePermissions.permission }).from(rolePermissions).where(eq(rolePermissions.roleId, roleId));
      ...
      const escalated = await this.findEscalatedGrants(actorRole.roleId, nextPermissionRecord);
      if (escalated.length > 0) return err("FORBIDDEN", `Cannot grant permissions you do not hold: ${escalated.join(", ")}`);
    }
    ...
    const [updatedRole] = await this.deps.db.select({ id: roles.id, name: roles.name, level: roles.level, color: roles.color, createdAt: roles.createdAt, updatedAt: roles.updatedAt }).from(roles).where(eq(roles.id, roleId)).limit(1);
    if (!updatedRole) return err("SERVER_ERROR", "Failed to load updated role");
    const permissionRows = await this.deps.db.select({ permission: rolePermissions.permission }).from(rolePermissions).where(eq(rolePermissions.roleId, roleId));
    ...
    await this.deps.writeAuditLogDurable({ entityType: "role", action: "update", actorId, entityId: roleId, diffTitle: updatedRole.name, detail: roleDiff });
    return ok(adminRoleSchema.parse({ id: updatedRole.id, name: updatedRole.name, level: updatedRole.level, color: updatedRole.color, created_at: updatedRole.createdAt, updated_at: updatedRole.updatedAt, permissions: parsePermissionRecord(permissionRows), assigned_user_count: assignedCount }));
  }

  async deleteRole(actorId: string, roleId: string): Promise<ServiceResult<void>> {

    const existing = (await this.deps.db.select({ id: roles.id, name: roles.name, level: roles.level }).from(roles).where(eq(roles.id, roleId)).limit(1))[0];
    if (!existing) return err("NOT_FOUND", "Role not found");
    const actorRole = await this.getActorRoleLevel(actorId);
    if (!actorRole) return err("FORBIDDEN", "Could not resolve actor role");
    if (existing.level >= actorRole.level) return err("FORBIDDEN", "Cannot delete a role at or above your own level");
    const assignedCountRow = (await this.deps.db.select({ count: sql<number>`count(*)` }).from(users).where(eq(users.role, roleId)).limit(1))[0];
    const assignedCount = Number(assignedCountRow?.count ?? 0);
    if (assignedCount > 0) return err("CONFLICT", "Role is assigned to users", { assigned_user_count: assignedCount });
    const inviteCountRow = (await this.deps.db.select({ count: sql<number>`count(*)` }).from(inviteLinks).where(eq(inviteLinks.roleId, roleId)).limit(1))[0];
    const inviteCount = Number(inviteCountRow?.count ?? 0);
    if (inviteCount > 0) return err("CONFLICT", "Role is assigned to invite links", { invite_link_count: inviteCount });
    try {
      await this.deps.db.delete(roles).where(eq(roles.id, roleId));
    } catch (error) {
      if (error instanceof Error && /FOREIGN KEY constraint failed/i.test(error.message)) return err("CONFLICT", "Role is still referenced");
      throw error;
    }
  ...
  private async assertBatchActionAllowed(
    ...
    | { ok: true; actorRole: { level: number; roleId: string }; existingUsers: { id: string; username: string; role: string; roleLevel: number }[] }
  ...
  > {
    ...
    const existingUsers = await this.deps.db
      .select({ id: users.id, username: users.username, role: users.role, roleLevel: roles.level })
      .from(users)
      .innerJoin(roles, eq(users.role, roles.id))
      .where(and(inArray(users.id, targetIds), isNull(users.deletedAt)));

    const protectedUsers = existingUsers.filter((u) => u.roleLevel >= actorRole.level);
  ...
  private async getGrantedPermissions(roleId: string): Promise<ReadonlySet<string>> {
    const rows = await this.deps.db.select({ permission: rolePermissions.permission }).from(rolePermissions).where(eq(rolePermissions.roleId, roleId));
  ...
  private async findEscalatedRoleGrants(actorRoleId: string, targetRoleId: string): Promise<Permission[]> {
    const requested = emptyPermissionRecord();
    const targetPermissions = await this.getGrantedPermissions(targetRoleId);
    for (const permission of targetPermissions) {
      if ((PERMISSIONS as readonly string[]).includes(permission)) requested[permission as Permission] = true;
    }
    return this.findEscalatedGrants(actorRoleId, requested);
  }

  private async assertRoleAssignable(
    actorId: string,
    targetRoleId: string,
  ): Promise<{ ok: true } | { ok: false; error: ServiceErr }> {
    const actorRole = await this.getActorRoleLevel(actorId);
    if (!actorRole) return { ok: false, error: err("FORBIDDEN", "Could not resolve actor role") };
    const targetRole = (await this.deps.db.select({ level: roles.level }).from(roles).where(eq(roles.id, targetRoleId)).limit(1))[0];
    if (!targetRole) return { ok: false, error: err("NOT_FOUND", "Role not found") };
    if (targetRole.level >= actorRole.level) return { ok: false, error: err("FORBIDDEN", "Cannot assign a role at or above your own level") };
    const escalated = await this.findEscalatedRoleGrants(actorRole.roleId, targetRoleId);
    if (escalated.length > 0) {
      return { ok: false, error: err("FORBIDDEN", `Cannot assign a role with permissions you do not hold: ${escalated.join(", ")}`) };
    }
    return { ok: true };
  }
  ...
  private getSiteConfigService() {
    return new SiteConfigService(this.deps.db, {
      mediaService: this.deps.mediaService,
