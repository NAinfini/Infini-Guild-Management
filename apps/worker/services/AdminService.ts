...
export class AdminService {
  ...
  async createInviteLink(actorId: string, roleId: string, maxUses: number, expiresAt: string | null): Promise<ServiceResult<unknown>> {
    ...
    await this.deps.writeAuditLog({ entityType: "invite_link", action: "create", actorId, entityId: inviteId, diffTitle: inviteId, detailText: JSON.stringify({ role_id: roleId, max_uses: maxUses, expires_at: expiresAt }) });
  ...
  async createMember(actorId: string, username: string, roleId: string): Promise<ServiceResult<{ user_id: string; username: string; temporary_password: string }>> {
    ...
    await this.deps.writeAuditLog({ entityType: "user", action: "admin_create_member", actorId, entityId: userId, diffTitle: username, detailText: JSON.stringify({ username, role_id: roleId }) });
  ...
  async listRoles(): Promise<ServiceResult<AdminRole[]>> {
    ...
    return ok(roleRows.map((r) => adminRoleSchema.parse({ id: r.id, name: r.name, level: r.level, color: r.color, created_at: r.createdAt, updated_at: r.updatedAt, permissions: parsePermissionRecord(permissionRows.filter((p) => p.roleId === r.id).map((p) => ({ permission: p.permission, granted: p.granted }))), assigned_user_count: assignedCountByRole.get(r.id) ?? 0 })));
  }
