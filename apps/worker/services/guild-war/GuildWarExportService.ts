...
function buildWarHistoryCsv(rows: WarHistoryRow[], creatorMap: Map<string, string>, teamStatKeys: string[]): string {
  ...
  for (const row of rows) {
    const payload = toWarHistoryPayload(row);
    const ownStats = payload.own_stats as Partial<Record<string, number | null>> | null;
    const enemyStats = payload.enemy_stats as Partial<Record<string, number | null>> | null;
    const statCells = teamStatKeys.flatMap((key) => [toCsvCell(ownStats?.[key]), toCsvCell(enemyStats?.[key])]);
...
export class GuildWarExportService extends GuildWarCoreService {
  ...
  async exportHistory(format: "csv" | "json", filters: { dateFrom?: string; dateTo?: string; eventId?: string }): Promise<ServiceResult<{ content: string; contentType: string; filename: string }>> {
    ...
    const rows = await this.db.select(WAR_HISTORY_FIELDS).from(warHistory).where(where.length > 0 ? and(...where) : undefined).orderBy(desc(warHistory.createdAt), desc(warHistory.id)).limit(5000);
