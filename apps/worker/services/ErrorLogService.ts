...
function toErrorLogValues(input: ErrorLogInput) {
  return {
    ...
    context: serializeJsonObject(input.context),
  };
}

export function toErrorLogResponse(row: typeof errorLog.$inferSelect) {
  return { ...row, context: parseJsonObject(row.context) };
}
