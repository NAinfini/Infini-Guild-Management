...
describe("GalleryService", () => {
  it("allows members to delete their own gallery items without gallery.delete", async () => {
    const { db, calls } = createDeleteDb({ id: "item-1", type: "image", url: "gallery/images/item-1", caption: null, uploadedBy: "u-1" });
  ...
  it("allows gallery.delete holders to delete any gallery item", async () => {
    const { db, calls } = createDeleteDb({ id: "item-1", type: "video", url: "https://example.com/video", caption: null, uploadedBy: "owner-1" });
    const service = new GalleryService(db as never, createDeps());
