...
> Status: Phase 0–3 implementation is present in this workspace but has not been committed, deployed, or released. [README.md](./README.md), [SETUP.md](./SETUP.md), and [SECURITY.md](./SECURITY.md) remain the operational references.
>
> [中文版本](./AUTHENTICATION.zh.md)
...
| Registration | Invite code, private login name, public display name, password, and password confirmation. No OAuth, email, or phone step. |
...
Separating the public display name from the private login name is worthwhile, but the login name is not a secret or a second factor. It reduces opportunistic guessing and credential stuffing only after a member chooses a login value that is no longer publicly displayed; migrated accounts initially retain their already-known value. Strong passwords, throttling, lock handling, session security, and safe recovery remain the actual security controls. Invite-only registration limits unauthorized account creation, not attacks against existing accounts.
...
1. A guild administrator creates an invite under the existing invite policy.
2. The registration page asks for the invite code, login name, display name, password, and password confirmation.
3. The server validates the invite before performing an availability check or password hashing.
4. One transaction consumes the invite, creates the internal user, creates local credentials, creates the profile, and writes the registration audit event. A collision or unavailable invite consumes nothing.
...
| `login_failures` | Persistent progressive lock state keyed by the normalized private login name, inspected before account lookup and password derivation. |
...
The current code uses self-describing PBKDF2-HMAC-SHA256 hashes with a 10,000-iteration compatibility floor because the Cloudflare Workers free CPU budget is restrictive. That number should be documented as a deployment constraint, not described as modern best strength.
...
- require 8 to 128 characters for new passwords, permit spaces and Unicode, and do not impose composition or weak-password matching rules; and
- grandfather existing hashes until the next password change/reset rather than locking out members at deployment.

If a site owner enables arbitrary-recipient Cloudflare Email Sending, that Cloudflare account is already on Workers Paid and should benchmark a higher PBKDF2 setting. VPS owners should do the same. A future move to another password hash must be runtime-neutral and use the self-describing migration path; Cloudflare and VPS must not silently implement different authentication strength.
...
- Inspect an active persistent login lock before account lookup or PBKDF2 work.
- Preserve a constant-shape dummy password derivation for unknown or unusable accounts when the request is not locked.
- Keep progressive persistent backoff, generic errors, bounded retention, permission-gated reset, and audit.
...
2. Copy every current `users.username` value into both fields without changing its text. Existing password hashes remain unchanged, and current login-failure state keeps the same key value.
...
7. Move persistent login-lock ownership to `login_name` and clear/transfer lock state atomically when that name changes.
...
- Move active-lock rejection ahead of account lookup and PBKDF2, matching the repository security contract.
- Add IP-wide plus IP/login-pair throttling while retaining persistent progressive locks and dummy derivation.
- Add the 8-to-128-character password policy for registration/change/reset, generic failure responses, and tests.
...
**Exit:** focused service/store/route tests pass, lock requests perform neither account lookup nor PBKDF2, and both runtimes enforce the same behavior.
...
- service tests for transactions, uniqueness, locking, replay, session revocation, and audit atomicity;
