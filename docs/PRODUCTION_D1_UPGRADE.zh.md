...
本次 0.1.0 将已经完成的 `0000`–`0017` 链合并成单一、只含最终状态的 `0000_core.sql`。这是明确授权的一次性切换，不授权自动修补账本；应用仍仅接受唯一精确 manifest。

**发布检查点（2026-08-30）：** 合并后，所有者要求仅提交 Git 和更新 release。生产仍运行提交 `250544ba`，应用账本保留已完成的 18 行；此次源码刷新没有执行生产账本切换或部署。完成下述维护期切换前，不得将合并源码部署到该数据库。

## 选择正确路径

- **全新数据库：** 从合并 core 初始化：68 张应用表、152 个命名索引、90 个触发器及权威种子，不再执行历史重建。
- **完整原 18 行账本数据库：** 不执行新 core，核对结构等价后按下述步骤在维护期切换应用账本。
- **更旧或部分迁移数据库：** 停止；先使用标签 `archive/pre-core-20260830` 完成旧链，并单独备份、演练。
...
本次依据所有者明确的本地验证豁免继续，CI 单独诊断；豁免不涵盖数据、备份或登录检查。媒体 HTTP 成功不能证明悬停音频可听，本次切换不宣称修复该独立问题。
