...
- Each card shows:
  ...
  - Participants: `Avatar.Group` showing first 5 participant avatars with `+N` overflow
  - Click avatar group opens full participant list
...
- Recurring templates are managed in a dedicated **Recurring Templates** workbench tab on `/events`
- Templates tab shows a list of all recurring series (parent events) with frequency, next instance date, total instances
- Create/Edit template: modal with title, description, type, recurrence rule (frequency + end condition)
- Instances are auto-generated by cron and appear in the main card list / calendar as regular events
- When editing a recurring instance from the card list: prompt "Edit this event only" vs "Edit all future events"
...
- Single view: **Month** calendar grid
...
- Mobile: month view with tap event pill → detail modal
