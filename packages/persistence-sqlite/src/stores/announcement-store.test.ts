...
describe("SqliteAnnouncementStore", () => {
  it("returns an empty list body projection while detail retains the full rich body", async () => {
    ...
    expect(page.data[0]?.body_json).toBe("");
  ...
  it("uses the public-list index without a temporary order B-tree", () => {
    ...
    const plan = database.prepare(`EXPLAIN QUERY PLAN
      SELECT id, title, '', pinned, status, publish_at, expires_at, archived_at,
