...
访客可访问 `/`、`/events`、`/roster`、`/announcements`、`/guild-war`、`/gallery`、`/wiki`、`/settings` 和 `/tools`。登录和邀请注册分别使用 `/login` 与 `/register`。`/profile`、`/storage`、`/storage/manage` 与 `/admin` 需要会话；高权限操作会由 API 再次校验。
...
Cloudflare 使用 Cron Triggers。VPS 则在单个 Node.js 进程中调度相同任务。媒体清理只选择已过期且没有关联的数据库资产，并删除记录中的精确 Blob key；不会从路径猜测归属，也不会把 Blob 扫描结果当作授权依据。
